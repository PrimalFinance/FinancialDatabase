...
class DataManager:
    ...
    def get_data(self, ticker: str, crypto: bool = False, overwrite: bool = False):
        ticker_file = f"Database\\EquityData\\Stocks\\{ticker}.csv"
        if overwrite:
    ...
    def get_cpi(self) -> pd.DataFrame:
        file_path = f"{macro_data_folder}\\CPI\\cpi.csv"
        data = pd.read_csv(file_path)
        # Update csv if outdated.
        if self.is_outdated(data["Date"].iloc[0]):
            self.macro_scraper.update_cpi()
            data = pd.read_csv(file_path)  # Read again after updating
        return data

    def get_fed_funds(self) -> pd.DataFrame:
        file_path = f"{macro_data_folder}\\fed_funds.csv"
        data = pd.read_csv(file_path)
        # Update csv if outdated.
        if self.is_outdated(data["Date"].iloc[0]):
            self.macro_scraper.update_fed_funds()
            data = pd.read_csv(file_path)  # Read again after updating
        return data

    def get_fed_funds(self) -> pd.DataFrame:
        file_path = f"{macro_data_folder}\\Treasury_Yield_Spread_10Y-2Y.csv"
        data = pd.read_csv(file_path)
        # Update csv if outdated.
        if self.is_outdated(data["Date"].iloc[0]):
            self.macro_scraper.update_treasury_yield_spread()
            data = pd.read_csv(file_path)  # Read again after updating
        return data
